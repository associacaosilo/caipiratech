...
//////////////////////////////
//config inicial usado só na loja
//////////////////////////////
...
//lista é criada no elemento com id listings
function buildLocationList(locationData) {
  ...
  //usa para marcar as palavras pesquisadas no resultado
...
function filtroProdutos(){
  ...
  if(value.length){    
    ...
    // for (let j = 0; j < produtosCinthia.length; j++) {
      ...
      // produtos2 = cleanString(produtos2);
...
function buildCheckbox(title, listItems) {
  ...
  for (let i = 0; i < listItems.length; i++) {
    ...
    inputValue.classList.add('cursor-pointer');
  ...
  // filterForm.addEventListener('change', function () {
    ...
    if(checkedBoxes.length == 0){      
      ...
      for (var checkbox of checkboxes) {        
          //document.getElementById(checkbox.id).setAttribute('checked', '');
          checkbox.checked = true;
          //checkbox.setAttribute('checked', '');
      }
    }
    //atualiiza o mapa com o geojson filtrado
    map.getSource('locationData').setData(filteredGeojson);

    if(isLoja==1){
    //atualiza lista de resultados com geojson filtrado
    buildLocationList(filteredGeojson);    
    //chama função para filtrar produtos de acordo com o texto digitado
    filtroProdutos();
    }
